...
#------------------------------------------------------------------------------
working_messages = ["I am tired!", "Could you substitude me for a moment?"]
...
def OnThink (self, val):
    ...
    if val == 4:
        ...
        if randrange (100) < 30:
               self.Emote (co.EMOTE_ONESHOT_TALK)
               self.Say (self, "Greetings stranger! Welcome to the Stormwind City!", co.LANG_UNIVERSAL)
 
    if val <= 3:
        self.Emote (co.EMOTE_ONESHOT_FLEX)
        val+=1
        self.NextThink (100000,val)
    else:
        val=1
        if randrange (100) < 5:
            self.Emote (co.EMOTE_ONESHOT_QUESTION)
            self.Say (self, choice (working_messages), co.LANG_UNIVERSAL)
            print "Say: Could you substitude me for a moment?"
