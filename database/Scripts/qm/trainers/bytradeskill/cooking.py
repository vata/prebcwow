#
# Default Cooking Trainers Module
#
# Author: <PavkaM>
...
def OnHello (self, player):
    ...
    if self.HasSellTables():
        player.AddGossipItem( 1, "I need recepies.", co.GOSSIP_SENDER_MAIN, co.GOSSIP_ACTION_TRADE)

    player.AddGossipItem( 3, "Train me.", co.GOSSIP_SENDER_MAIN, co.GOSSIP_ACTION_TRAIN)
    player.SendGossipMenu(self, 2676) 
...
def OnGossipSelect (self, player, sender, action):
    ...
    if action == co.GOSSIP_ACTION_TRAIN :
        player.SendTrainerList(self, "Can I teach you how to turn the meat you find on beasts into a feast?")
