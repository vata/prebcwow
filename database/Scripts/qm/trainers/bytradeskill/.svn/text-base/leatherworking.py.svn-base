#
# Default LeatherWorking Trainers Module
#
# Author: <PavkaM>
...
def OnHello (self, player):
    ...
    if self.HasSellTables():
        player.AddGossipItem( 1, "I need leatherworking supplies.", co.GOSSIP_SENDER_MAIN, co.GOSSIP_ACTION_TRADE)

    player.AddGossipItem( 3, "Train me.", co.GOSSIP_SENDER_MAIN, co.GOSSIP_ACTION_TRAIN)
    player.SendGossipMenu(self, 5257) 
...
def OnGossipSelect (self, player, sender, action):
    ...
    if action == co.GOSSIP_ACTION_TRAIN :
        player.SendTrainerList(self, "Greetings!  Can I teach you how to turn beast hides into armor?")
