...
if [ -d "./src" -a -d "./dep" -a -d "./build" -a -d "./.svn" ]; then
 ...
 echo "(Error) Not Running in CRFusion Directory !!!!!!"
 exit
fi
...
#endif
" > src/CRFusion/Version/revision.h
...
cd "$CDR"
scons -s || { echo "CRFusion Compilation faild !"; exit; }

echo " ** CRFusion Succesefully Compiled !"
echo "Step 4. Post-build actions"

NMNEW="CRFusion-`uname`-`uname -i`.bin"
cp ./build/CRFusion $NMNEW
rm $NMNEW.gz
gzip $NMNEW
