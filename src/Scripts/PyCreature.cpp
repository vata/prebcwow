...
Py::Object PyUnit::ext_Say (const Py::Tuple& args)
{
	...
	if (cr != NULL)
	{
		...
		cr->Say (receiver.extensionObject()->GetUnit()->GetGUID(), text.as_std_string().c_str(),
...
Py::Object PyUnit::ext_SetUpdateFieldValue (const Py::Tuple& args)
{
	...
	if (p != NULL)
	{
		...
		case 6: p->SetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS, (uint32)value); break;
		case 7: p->SetUInt32Value(PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS, (uint32)value); break;
...
Py::Object PyUnit::ext_GetUpdateFieldValue (const Py::Tuple& args)
{
	...
	if (p != NULL)
	{
		...
		case 6: value = p->GetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS); break;
		case 7: value = p->GetUInt32Value(PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS); break;
...
//-----------------------------------------------------------------------------
Py::Object PyUnit::ext_GetSecurity (const Py::Tuple& args)
{
	args.verify_length (0);

	Player* p = GetPlayer();
	if (p != NULL)
		return Py::Int (int(p->GetSession()->GetSecurity()));
