...
//
class CRFusionPy: public Py::ExtensionModule<CRFusionPy>
{
public:
	CRFusionPy():
	  Py::ExtensionModule<CRFusionPy>("CRFusion")
	  {
		  // Damn crap forgot to init this shit
		  PyUnit::init_type();
		  //PyCreature::init_type();

		  add_varargs_method ("RegisterAIModule", &CRFusionPy::ext_RegisterAIModule,
			  "(creatureId, modulePath) Registers script module for specific creature Id.");

		  add_varargs_method ("RegisterGossipModule", &CRFusionPy::ext_RegisterGossipModule,
			  "(npcId, modulePath, showlog) Registers gossip script module for specific npc Id.");

		  add_varargs_method ("RegisterItemHandlerModule", &CRFusionPy::ext_RegisterItemHandlerModule,
			  "(itemId, modulePath, showlog) Registers handler script module for specific item Id.");

		  add_varargs_method ("RegisterGOHandlerModule", &CRFusionPy::ext_RegisterGOHandlerModule,
			  "(goId, modulePath, showlog) Registers handler script module for specific go Id.");

		  add_varargs_method ("RegisterAreaTriggerModule", &CRFusionPy::ext_RegisterAreaTriggerModule,
			  "(triggerId, modulePath, showlog) Registers handler script module for specific trigger Id.");

		  add_varargs_method ("RegisterSpellClass", &CRFusionPy::ext_RegisterSpellClass,
			  "(spellClass) Registers class with handler functions for specific spell Id.");

		  add_varargs_method ("RegisterStoredNPCText", &CRFusionPy::ext_RegisterNPCTextAssoc,
			  "(blockid, npcId, npctextId) Sets a default text for a specified action.");

		  add_varargs_method ("RegisterStoredText", &CRFusionPy::ext_RegisterDefaultTextAssoc,
			  "(blockid, npcId, text) Sets a default text for some action. See an example");

		  add_varargs_method ("RequireServerBuild", &CRFusionPy::ext_RequireServerBuild,
			  "(ver) Returns true if the specified CRFusion ver. is compatible with this one.");
		  initialize( "CRFusion is the base module for all scripts in the server." );
	  }
...
//
// Here go implementated functions
//

char *
GetTextAssoc(unsigned int creatureId, unsigned int AssocId)
{
...
uint32
GetNPCTextAssoc(unsigned int creatureId, unsigned int AssocId)
{
	if (AssocId > ASSOC_TEXT_ENTRIES) return NULL;
...
AIModuleInfo *
GetAIModule (unsigned int creatureId)
{
...
GossipModuleInfo *
GetGossipModule (unsigned int objId)
{
...
QuestItemModuleInfo *
GetQuestItemModule (unsigned int objId)
{
...
QuestGOModuleInfo *
GetQuestGOModule (unsigned int objId)
{
...
GetQuestTriggerModule (unsigned int objId)
{
...
SpellHandlerInfo *
GetSpellHandler (unsigned int spellId)
{
...
#endif
	static CRFusionPy * static_lud_python = new CRFusionPy;
}
...
InitScripting()
{
	...
	sLog.outColor (4);
	sLog.outBasic ("[Python] AI module...");
	sLog.outColor (0);
	...
	sLog.outColor (4);
	sLog.outBasic ("[Python] Gossip/Quest module...");
	sLog.outColor (0);
	...
	sLog.outColor (4);
	sLog.outBasic ("[Python] Spells module...");
	sLog.outColor (0);
	...
	sLog.outColor (4);
	sLog.outBasic ("[Python] Races and Classes module...");
	sLog.outColor (0);
	...
	sLog.outColor (4);
    sLog.outBasic ("[Python] Executables module...");
	sLog.outColor (0);
	...
	sLog.outColor (4);
	sLog.outBasic ("[Python] Functions module...");
	sLog.outColor (0);
	...
	sLog.outColor (4);
	sLog.outBasic ("[Python] scripting initialized ->>");
	sLog.outBasic ("");
	sLog.outColor (2);
...
ShutdownScripting()
{
	Py_Finalize();
	sLog.outColor (3);
	sLog.outBasic ("[Python] scripting shut down ->>>");
	sLog.outBasic ("");
	sLog.outColor (2);
}
...
RestartScripting()
{
	...
	sLog.outBasic ("");
	sLog.outColor (3);
	sLog.outBasic ("[Python] Restarting scripting system...");
	...
	sLog.outBasic ("[Python] Refreshing AI scripts");
	p = PyImport_ImportModuleEx ("startup_ai", **g_AiGlobals, NULL, NULL);
	...
	sLog.outBasic ("[Python] Refreshing Gossip/Quest scripts");
	p = PyImport_ImportModuleEx ("startup_qm", **g_AiGlobals, NULL, NULL);
	PyImport_ReloadModule (p);
	if (PyErr_Occurred() != NULL) {
		PyErr_Print(); PyErr_Clear();
	...
	sLog.outBasic ("[Python] Refreshing Spells scripts");
	p = PyImport_ImportModuleEx ("startup_spells", **g_AiGlobals, NULL, NULL);
	PyImport_ReloadModule (p);
	if (PyErr_Occurred() != NULL) {
		PyErr_Print(); PyErr_Clear();
	...
	sLog.outBasic ("[Python] Refreshing Races and Classes scripts");
	p = PyImport_ImportModuleEx ("player_class", **g_AiGlobals, NULL, NULL);
	PyImport_ReloadModule (p);
	if (PyErr_Occurred() != NULL) {
		PyErr_Print(); PyErr_Clear();
	...
	sLog.outBasic ("[Python] Refreshing Executables");
	p = PyImport_ImportModuleEx ("executables", **g_AiGlobals, NULL, NULL);
	PyImport_ReloadModule (p);
	if (PyErr_Occurred() != NULL) {
		PyErr_Print(); PyErr_Clear();
	...
	sLog.outBasic ("[Python] Refreshing Functions");
	p = PyImport_ImportModuleEx ("functions", **g_AiGlobals, NULL, NULL);
	PyImport_ReloadModule (p);
	if (PyErr_Occurred() != NULL) {
		PyErr_Print(); PyErr_Clear();
...
void
ShutdownThreadScripting (void *state)
{
...
Py::Callable *
_getModuleFunc (Py::Module & module, char * name)
{
...
Py::Object
CRFusionPy::ext_RequireServerBuild (const Py::Tuple &param)
{
...
Py::Object
CRFusionPy::ext_RegisterAIModule (const Py::Tuple &param)
{
...
Py::Object
CRFusionPy::ext_RegisterNPCTextAssoc (const Py::Tuple &param)
{
	param.verify_length (3);
...
Py::Object
CRFusionPy::ext_RegisterDefaultTextAssoc (const Py::Tuple &param)
{
	param.verify_length (3);
...
Py::Object
CRFusionPy::ext_RegisterGossipModule (const Py::Tuple &param)
{
	param.verify_length (3);
	...
	{
		if (pCreature)
			printf("  QM WARNING: NPC '%u' : '%s/%s', already registered with module '%s'.\n", (unsigned int)pyObjId, pCreature->Name.c_str(), pCreature->Guild.c_str(), info->Name.c_str());
		else
...
Py::Object
CRFusionPy::ext_RegisterItemHandlerModule (const Py::Tuple &param)
{
	param.verify_length (3);
...
Py::Object
CRFusionPy::ext_RegisterGOHandlerModule (const Py::Tuple &param)
{
	param.verify_length (3);
...
Py::Object
CRFusionPy::ext_RegisterAreaTriggerModule (const Py::Tuple &param)
{
	param.verify_length (3);
...
void
Call_Unit_OnGossipHello (Unit *self, Unit *player)
{
...
uint32
Call_Unit_OnDialogStatus (Unit *self, Unit *player)
{
	GossipModuleInfo *pDefInfo = GetGossipModule ( 0 );
	GossipModuleInfo *pModInfo = GetGossipModule (self->GetEntry());
...
void
Call_Unit_OnChooseReward (Unit *self, Unit *player, uint32 questid, uint32 optid)
{
	GossipModuleInfo *pDefInfo = GetGossipModule ( 0 );
	GossipModuleInfo *pModInfo = GetGossipModule (self->GetEntry());
...
void
Call_Unit_OnGossipSelect (Unit *self, Unit *player, uint32 senderMenu, uint32 action)
{
	GossipModuleInfo *pDefInfo = GetGossipModule ( 0 );
	GossipModuleInfo *pModInfo = GetGossipModule (self->GetEntry());
...
void
Call_Unit_OnGossipSelectCode (Unit *self, Unit *player, uint32 senderMenu, uint32 action, char *code)
{
	GossipModuleInfo *pDefInfo = GetGossipModule ( 0 );
	GossipModuleInfo *pModInfo = GetGossipModule (self->GetEntry());
...
void
Call_Unit_OnQuestAccept (Unit *self, Unit *player, uint32 questid)
{
	GossipModuleInfo *pDefInfo = GetGossipModule ( 0 );
	GossipModuleInfo *pModInfo = GetGossipModule (self->GetEntry());
...
void
Call_Unit_OnQuestComplete (Unit *self, Unit *player, uint32 questid)
{
	GossipModuleInfo *pDefInfo = GetGossipModule ( 0 );
	GossipModuleInfo *pModInfo = GetGossipModule (self->GetEntry());
...
void
Call_Unit_OnQuestSelect (Unit *self, Unit *player, uint32 questid)
{
	GossipModuleInfo *pDefInfo = GetGossipModule ( 0 );
	GossipModuleInfo *pModInfo = GetGossipModule (self->GetEntry());
...
void
Call_Item_OnSelect (Item *item, Unit *player, uint32 questid)
{
...
void
Call_Item_OnQuestAccept (Item *item, Unit *player, uint32 questid)
{
	QuestItemModuleInfo *pDefInfo = GetQuestItemModule ( 0 );
	QuestItemModuleInfo *pModInfo = GetQuestItemModule (item->GetEntry());
...
void
Call_Obj_OnSelect (GameObject *go, Unit *player)
{
...
void
Call_Trigger_OnSelect (Unit *player, uint32 atid, uint32 questid)
{
...
void
Call_Obj_OnChooseReward (GameObject *self, Unit *player, uint32 questid, uint32 optid)
{
	QuestGOModuleInfo *pDefInfo = GetQuestGOModule ( 0 );
...
void
Call_Obj_OnQuestAccept (GameObject *self, Unit *player, uint32 questid)
{
	QuestGOModuleInfo *pDefInfo = GetQuestGOModule ( 0 );
	QuestGOModuleInfo *pModInfo = GetQuestGOModule (self->GetEntry());
...
void
Call_Unit_OnAttacked (Unit *self, Unit *attacker)
{
...
void
Call_Unit_OnTakeDamage (Unit *self, Unit *attacker)
{
	AIModuleInfo *pModInfo = GetAIModule (self->GetEntry());
	if (pModInfo == NULL) return;
...
void
Call_Unit_OnThink (Unit *self, uint32 param)
{
	AIModuleInfo *pModInfo = GetAIModule (self->GetEntry());
	if (pModInfo == NULL) return;
...
void
Call_Unit_OnWaypoint (Unit *self, int value)
{
	AIModuleInfo *pModInfo = GetAIModule (self->GetEntry());
	if (pModInfo == NULL) return;
	if (pModInfo->func_OnInitNpc == NULL) return;
...
Py::Callable *
_getObjectFunc (Py::Object & obj, char * name)
{
	if (obj.hasAttr (name)) {
...
Py::Object
CRFusionPy::ext_RegisterSpellClass (const Py::Tuple &param)
{
	...
	g_spellMap[(uint32)pySpellId] = info;

	return Py::None();
}

uint32
Call_Spell_CanCast (uint32 spellId, Unit *caster, Unit *target)
{
...
bool
Call_Spell_SpellEffect (uint32 spellId, Unit *caster, Unit *target)
{
	SpellHandlerInfo *pModInfo = GetSpellHandler (spellId);
...
PyObject *
GetAuraHandler (unsigned int auraId)
{
...
Py::Object
CRFusionPy::ext_RegisterAuraHandler (const Py::Tuple &param)
{
	param.verify_length (2);
...
bool
Call_Aura_ApplyModifier (uint32 auraId, Unit *caster, Unit *target, bool apply,
...
//
//  CHAR INITIALIZATION BY RACE AND CLASS
//
void
_Call_RaceClassInit (char *funcName, Unit *p, bool oneTimeInit)
{
	PyEval_AcquireLock_Macro;
...
void
Call_PlayerStats_ByRace  (Unit *p, bool oneTimeInit)
{
	uint8	rc = p->GetRace();
	switch (rc)
	{
...
void
Call_PlayerStats_ByClass (Unit *p, bool oneTimeInit)
{
...
uint32
_Call_Functions (char *funcName, Unit *self, Unit *victim)
{
	PyEval_AcquireLock_Macro;
...
void
_Call_Executables (char *funcName, Unit *self, Unit *victim)
{
	PyEval_AcquireLock_Macro;
...
uint32
Call_CalcXP (Unit *self, Unit *victim)
{
	return _Call_Functions ("CalcXP", self, victim);
}
uint32
Call_CalcDR (Unit *self, Unit *victim, uint32 damages)
{	
	PyEval_AcquireLock_Macro;

	Py::ExtensionObject<PyUnit> pySelf (new PyUnit (self));
	Py::ExtensionObject<PyUnit>	pyVictim (new PyUnit (victim));
	Py::Int						pyDamages(new int (damages));
	...
	Py::Callable pyProc (module.getAttr ("CalcDR"));
	Py::Object result (pyProc.apply (args));
	if (PyErr_Occurred() != NULL) {
		PyErr_Print();
		PyErr_Clear();
...
void
Call_CalcReputation (Unit *killer, Unit *victim)
{
	return _Call_Executables ("CalcReputation", killer, victim);
}

void
Call_CalcHonor (Unit *self, Unit *victim)
{
