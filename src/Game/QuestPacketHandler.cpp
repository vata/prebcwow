/*
 *  **********  (c) 2005 PavkaM. CRFusion Project *************
 */
...
{
	sLog.outDebug("Sent SpiritHealer List !");
...
{
	sLog.outDebug("Sent Tabard List !");
...
{
	sLog.outDebug("Sent Banker List !");
...
void QuestPacketHandler::SendTrainerList( WorldSession *Wrld, uint64 creatureGUID, std::string trtext)
{
	...
	sLog.outDebug("TRAIN: Sending Trainer List...");
	...
	//check for class correctnes
	bool correct_class = strainer->charclass == 0 ? true : false;

	if(strainer->charclass == Wrld->GetPlayer()->GetClass()) 
		correct_class = true;

	if(correct_class == false)
	{
		cTrainer->Say(Wrld->GetPlayer()->GetGUID(),"Do I look like $C trainer to you!");
		sLog.outDebug("TRAIN::Trainer with id [%d] can't train this player", cTrainer->GetEntry());
        return;
	}
	
	sLog.outDebug("TRAIN::Trainer with id [%d]", cTrainer->GetEntry());
	...
	if(strainer)
	{
		...
		sLog.outDebug("TRAIN: Trainer has spells to train.");
        ...
        for (uint32 t = 0;t < sSkillStore.GetNumRows(); ++t)
        {
			...
			{
				//sLog.outString("skill %u with skillline %u matches",skill->spell,skill->skilline);
		...
		sLog.outString("TRAIN: Spells count: %u", cnt);
        for (unsigned int t = 0;t < sSkillStore.GetNumRows(); ++t)
        {
				...
				|| (skill->skilline == strainer->skilline20))
			{
				...
				if ((proto) && (proto->spellLevel != 0) && (objmgr.TeachSpellID[skill->spell] != NULL))
				{
					sLog.outString( "TRAIN: Grabbing trainer spell %u with skilline %u", skill->spell, skill->skilline);
		...
		sLog.outDebug("TRAIN: Sent Trainer List !");
		Wrld->SendPacket( &data );
	}
}
...
{
	sLog.outDebug("Sent Petition List !");
...
{
	sLog.outDebug("Sent Auction List !");
...
void QuestPacketHandler::SendInnkeeperList( WorldSession *Wrld, uint64 creatureGUID)
{
    ...
    sLog.outDebug("Sent Inkeeper List !");
...
{
	sLog.outDebug("Sent Vendor List !");
	...
	if (iters == objmgr.m_sellTemplates.end()) {
		sLog.outDetail( "Erm NPC %d (GUID %X) has nothing to sell",
    ...
    for (uint8 itemcount = 0; itemcount < selltemp.size(); ++itemcount )
    {
		...
		if( !curItem ) {
            sLog.outError( "Unit %i has nonexistant item %i!", creatureGUID, selltemp[itemcount] );
            sLog.outBasic( "WORLD: DID NOT Send SMSG_LIST_INVENTORY Message" );
            for( int a = 0; a < 7; ++a )
                data << uint32( 0 );
        } else 
        {
