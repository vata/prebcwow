...
void
DynamicObject::Create( uint32 guidlow, Unit *caster, SpellEntry * spell, float x, float y, float z, uint32 duration )
{
...
void
DynamicObject::Update(int32 p_time)
{
...
DynamicObject::DealDamage()
{
	...
	for (MapRangeIterator itr (m_caster); o = itr.Advance(); )
    {
		...
		//_CalcDistance (GetPositionX(), GetPositionY(), GetPositionZ(),
		//	o->GetPositionX(), o->GetPositionY(), o->GetPositionZ())

		if (GetDistanceSq (o) < m_PeriodicDamageRadius * m_PeriodicDamageRadius &&
			o->GetUInt32Value (UNIT_FIELD_FACTIONTEMPLATE) != m_caster->GetFaction())
		{
            m_caster->PeriodicAuraLog ((Unit *)o, m_spell->Id, m_PeriodicDamage, m_spell->School);
        }
    }
}
