    ...
    while (!_recvQueue.empty())
    {
        ...
        for (i = 0; table[i].handler != NULL; ++i)
        {
            if (table[i].opcode == packet->GetOpcode())
            {
                ...
                {
                    sLog.outError( "SESSION: recieved unexpected opcode %s (0x%.4X)",
        ...
        if (table[i].handler == NULL)
            sLog.outError( "SESSION: recieved unhandled opcode %s (0x%.4X)",
                LookupName(packet->GetOpcode(), g_worldOpcodeNames),
                packet->GetOpcode());
...
OpcodeHandler* WorldSession::_GetOpcodeHandlerTable() const
{
    static OpcodeHandler table[] =
    {
		...
		{ MSG_RANDOM_ROLL,               STATUS_LOGGEDIN, &WorldSession::HandleRandomRoll                    },

		{ CMSG_SAVE_PLAYER,              STATUS_LOGGEDIN, &WorldSession::HandleSavePlayerOpcode              },
		...
		{ MSG_MOVE_TELEPORT_ACK,         STATUS_LOGGEDIN, &WorldSession::HandleMoveWorldportAckOpcode        },
		...
		// Warden & Hardware Survey Opcodes
		{ CMSG_WARDEN_DATA,              STATUS_LOGGEDIN, &WorldSession::HandleWardenInfo                    },
		{ CMSG_HARDWARE_SURVEY_RESULTS,  STATUS_LOGGEDIN, &WorldSession::HandleHardwareSurvey                },
...
//-----------------------------------------------------------------------------
void WorldSession::GMSysMessage (char *format, ...) {
	if(_security>0)
	{
		va_list params;
		va_start (params, format);
...
//-----------------------------------------------------------------------------
void WorldSession::WideScreenMessage (char *format, ...) {
	va_list params;
	va_start (params, format);
