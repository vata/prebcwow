...
void
Container::Create( uint32 guidlow, uint32 itemid, Player *owner )
{
	ItemPrototype *proto = objmgr.GetItemPrototype(itemid);
    ASSERT(proto);
	
	//Item::Create(guidlow, itemid, owner);
	...
	SetUInt32Value( ITEM_FIELD_FLAGS, proto->Flags);
	...
	sLog.outDebug ("CONTAINER: Create : ID[%d] SLOTS[%d]", itemid, proto->ContainerSlots);
