...
public:
	...
	// Some value for storing between applying and removing mod
	uint32 m_v1;
	void SetValue1 (uint32 v1) { m_v1 = v1; }
	uint32 GetValue1() { return m_v1; }
...
public:
    ...
    Affect() :
	m_spellProto(0), m_duration(0), m_casterGUID(0), m_damagePerTick(0),
	m_auraSlot(0), m_spellPerTick(0), m_coAffect(0)
	{
	}

    Affect(SpellEntry *proto, int32 duration, const uint64 &guid) :
    m_spellProto(proto), m_duration(duration), m_casterGUID(guid),
	m_damagePerTick(0), m_auraSlot(0), m_spellPerTick(0), m_coAffect(0) 
	{
	}
    ...
    uint32 GetSpellId() const { 
        if(m_spellProto)
            return m_spellProto->field167;  // changed due to column shift in 1.8.0 +3 col
        else
            return NULL;
    }
    uint32 GetId() const { 
    ...
    virtual void SetDuration(int32 duration) { m_duration = duration; }
    ...
    uint16 GetDamagePerTick() const { return m_damagePerTick; }
	...
	// For comparing affects that differ only by amount
	bool IsSameClassAffect (Affect *aff)
	{
		if (GetSpellId() != aff->GetSpellId()) return false;
		if (m_modList.size() != aff->GetModList().size()) return false;

		ModList::const_iterator i = m_modList.begin();
		ModList::const_iterator j = aff->GetModList().begin();
		for (; i != m_modList.end() && j != aff->GetModList().end();
			++i, ++j)
		{
			const Modifier &m1 = *i;
			const Modifier &m2 = *j;
			if (m1.GetType() != m2.GetType()) return false;
			if (m1.GetMiscValue() != m2.GetMiscValue()) return false;
		}
