...
void
Guild::create(uint64 lGuid, std::string gname)
{
	Player *pl;
	MemberSlot *newmember;
	std::string rname;

	pl = objmgr.GetObject<Player>(lGuid);
	if(!pl) return;
...
void
Guild::LoadGuildFromDB(uint32 GuildId)
{
...
void
Guild::LoadRanksFromDB(uint32 GuildId)
{
	std::stringstream query, ss;
	Field *fields;
...
void
Guild::LoadMembersFromDB(uint32 GuildId)
{
	...
	do
	{
		...
		pl = objmgr.GetObject<Player>(newmember->guid);
...
void
Guild::Loadplayerstats(MemberSlot *memslot)
{
	Field *fields;
	std::stringstream ss;
...
void
Guild::Loadplayerstatsbyguid(uint64 guid)
{
	Field *fields;
	Player *pl;
	std::stringstream ss;
	...
	for (itr = members.begin(); itr != members.end();itr++)
	{
	    if ((*itr)->guid == guid)
	    {
	    	pl = objmgr.GetObject<Player>((*itr)->guid);
...
void
Guild::SetPNOTE(uint64 guid,std::string pnote)
{
	std::list<MemberSlot*>::iterator itr;
	for (itr = members.begin(); itr != members.end();itr++)
	{
...
void
Guild::SetOFFNOTE(uint64 guid,std::string offnote)
{
	std::list<MemberSlot*>::iterator itr;
	for (itr = members.begin(); itr != members.end();itr++)
	{
...
void
Guild::SaveMemberToDB(MemberSlot *memslot)
{
	std::stringstream ss;
	ss << "DELETE FROM guilds_members WHERE memguid = '" << memslot->guid << "'";
	...
	ss << "INSERT INTO guilds_members VALUES ("
	   ...
	   << memslot->Pnote << "', '"
	   << memslot->OFFnote << "')";
	sLog.outDebug( "query member: %s", ss.str( ).c_str( ) );

	sDatabase.Execute( ss.str( ).c_str( ) );
}

void
Guild::DelGuildFromDB()
{
	sDatabase.PExecute("DELETE FROM guilds WHERE guildId = '%u'",Id);
	sDatabase.PExecute("DELETE FROM guilds_ranks WHERE guildId = '%u'",Id);
}
...
void
Guild::DelMemberFromDB(uint64 guid)
{	
	sDatabase.PExecute("DELETE FROM guilds_members WHERE memguid = '%d'",guid);
}

void
Guild::BroadcastToGuild(WorldSession *session, std::string msg)
{	
...
void
Guild::BroadcastToOfficers(WorldSession *session, std::string msg)
{
...
void
Guild::CreateRank(std::string name,uint32 rights)
{
	RankInfo *newrank;
...
void
Guild::SetRankName(uint32 rankId, std::string name)
{
	std::list<RankInfo*>::iterator itr;
...
void
Guild::SetRankRights(uint32 rankId, uint32 rights)
{
	std::list<RankInfo*>::iterator itr;
...
void
Guild::Roster(WorldSession *session)
{
	WorldPacket data;
	Player *pl;
...
void
Guild::Query(WorldSession *session)
{
	WorldPacket data;
