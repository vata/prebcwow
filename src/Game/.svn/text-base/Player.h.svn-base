...
typedef PlayerSet::iterator PlayerSetIter;
//====================================================================
//  Inventory
...
typedef stdext::hash_map<uint32, uint16> SpellMap;
typedef stdext::hash_map<uint32, uint32> SkillMap;
...
enum PlayerFlags
{
	...
	PLAYER_FLAG_PVP             = 0x03,
	PLAYER_FLAG_PLAYED_3HOURS	= 0x04,
	PLAYER_FLAG_PLAYED_5HOURS	= 0x05,
	PLAYER_FLAG_GM				= 0x08,
	PLAYER_FLAG_DEAD			= 0x10,
	PLAYER_FLAG_RESTING			= 0x20,
	PLAYER_FLAG_PVP_FREEFORALL	= 0x80,
...
public:
	...
	*/
	...
	// Faction/Team stuff
	//---------------------
	void SetPlayerFaction(uint8 race, uint32 faction);
	void SetPlayerTeam(uint32 team) { m_team = team; };
	uint32 GetPlayerFaction() { return m_faction; };
	uint32 GetTeam() { return m_team; };

	//---------------
	// Spells Stuff
	//---------------
	void TeleportToBindpoint();
	...
	bool HasNextSkill (uint32 skill_id) { return m_next_skill.find (skill_id) != m_next_skill.end(); }
	void DelNextSkill (uint32 skill_id) { m_next_skill.erase (skill_id); }

	uint32 GetNextSkill (uint32 skill_id)
	{
		SkillMap::iterator ii = m_next_skill.find (skill_id);
		if (ii != m_next_skill.end()) return uint32(ii->second);
		return 0;
	}
	
	//skill helpers
	uint16 GetSkillLevel (uint32 skill_id)	{ return m_skills_cur[skill_id]; }
	uint16 GetSkillMax (uint32 skill_id)	{ return m_skills_max[skill_id]; }

	bool HasSkill (uint32 skill_id) { return m_skills.find (skill_id) != m_skills.end(); }
	...
	void SetNextSkill (uint32 skill_id,uint32 lvl);

	// To remove skill - set its value to 0
	virtual bool AddSkill (uint32 skill_id, uint16 skillLevel = 1, uint16 maxSkill = 0xFFFF);
	virtual bool UpdateSkill (uint32 skill_id, uint16 skillLevel = 1, uint16 maxSkill = 0xFFFF);
	...
	void SetDuelOpponent(Player* Opponent) { m_pDuel = Opponent; }
	void SetDuelSender(Player* Sender) { m_pDuelSender = Sender; }
	void SetDuelFlag(GameObject* DuelFlag) { m_pDuelArbiter = DuelFlag; }

	bool isDueling() { return m_isInDuel; }
	Player* GetDuelOpponent() { return m_pDuel; }
	Player* GetDuelSender() { return m_pDuelSender; }
	GameObject* GetDuelFlag() { return m_pDuelArbiter; }

	//---------------------------
    //  Inventory operations
	//---------------------------
    bool SwapItemSlots(uint8 srcslot, uint8 dstslot);
    Item* GetItemBySlot(uint8 slot) { return m_items[slot]; }
	Item* GetItemByGUID(uint64 guid);
	...
	void LoadFromDBLight(uint32 guid);
    void DeleteFromDB();
	...
	void TeleportNear (float x, float y, float z, float o =0);
	void TeleportFar (uint32 mapId, float x, float y, float z, float o =0);
	...
	int32 m_Duel_timer;		//Player DUEL timer
	...
	// Easy Waypoints system :)
	bool isAddingWP() { return m_isAddingWP; }
	PathPoint GetLastAddedWP() { return m_LastAddedWP; }
	bool isTimeToAddWP(){
		float	cDist = m_eWP_Dist,
				Px = GetPositionX(),
				Py = GetPositionY(),
				Pz = GetPositionZ(),
				Wx = m_LastAddedWP.x,
...
protected:
	...
	bool m_isAddingWP;
	uint32 m_eWP_Settings[3]; // CreatureId, Wait1, Wait2
	PathPoint m_LastAddedWP; // X, Y, Z
	float m_eWP_Dist; // Distance between WayPoints in meters
	...
	SkillMap m_skills;
	SkillMap m_skills_max; // max skill
	SkillMap m_skills_cur; // current skill
	// Player Skills - dictionary key SkillID, when he levels
	SkillMap m_next_skill;
