...
public:
    ...
    virtual void Create( uint32 guidlow, uint32 itemid, Unit* owner );
	...
	Creature* GetCreatureOwner() const { return c_owner; }
	void SetCreatureOwner(Creature *owner) { c_owner = owner; }

	uint32 GetCount() { return GetUInt32Value (ITEM_FIELD_STACK_COUNT); }
	
	void SetCount (uint32 value) { 
		if (m_itemProto->InventoryType != INVTYPE_BAG) SetUInt32Value (ITEM_FIELD_STACK_COUNT, value); 
	}

	void UpdateStats();

	uint32 GetEntry() { return m_itemProto->ItemId; }
	...
	uint32 GetSkillNeeded(ItemPrototype * proto);
