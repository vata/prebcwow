...
private:
	...
	void MakeYouJoined(WorldPacket *data, Player *p) {
		*MakeNotifyPacket(data,0x02) << p->GetGUID(); 
		//(uint32)(players.size() > 1 ? 0x01 : 0x00);
	}
	...
	void MakeModeChange(WorldPacket *data, Player *who, char moderator, char voice) // 0 - no change, 1 - lost, 2 - got
	{
...
public:
	...
	void SetName(string newname) {
		name = newname;
		SetConstant (IsGeneralChannel());
	}
	string GetName() { return name; }

	bool IsGeneralChannel() {
		char cname[256];
		strcpy (cname, name.c_str());
		strlwr (cname);
		if(	strstr(cname, "general - ") || strstr(cname, "trade - ") || 
			strstr(cname, "localdefense - ") || strstr(cname, "lookingforgroup - ") ||
			strstr(cname, "général - ") || strstr(cname, "commerce - ") ||
			strstr(cname, "défenselocale - ") || strstr(cname, "recherchegroupe - ")
			)
			return true;
		else
            return false;
