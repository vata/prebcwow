...
void
WorldSession::HandleAuctionListBidderItems( WorldPacket & recv_data )
{
        uint64 guid;
...
void
WorldSession::HandleAuctionPlaceBid( WorldPacket & recv_data )
{
	uint64 auctioneer;
	...
	if ((ah) && (ah->owner != pl->GetGUIDLow()))
	{
		if ((price < ah->buyout) || (ah->buyout == 0))
		{
			...
			stringstream ss;
			ss << "INSERT INTO mail (mailId,sender,reciever,subject,body,item,"
				"time,money,COD,checked) VALUES ( " << n->messageID << ", "
				<< n->sender << ", " << n->reciever << ",' " << n->subject.c_str()
				<< "' ,' " << n->body.c_str() << "', " << n->item << ", " << (uint32)n->time
				<< ", " << n->money << ", " << n->COD << ", " << n->checked << " )";
			...
			else
			{
				...
				mi << "INSERT INTO mail (mailId,sender,reciever,subject,body,item,"
					...
					<< pl->GetGUIDLow() << ", " << m->reciever << ",' " << m->subject.c_str()
				...
				stringstream min;
				min << "INSERT INTO mail (mailId,sender,reciever,subject,body,item,"
					"time,money,COD,checked) VALUES ( " << mn->messageID << ", "
					<< mn->sender << ", " << mn->reciever << ",' " << mn->subject.c_str()
					<< "' ,' " << mn->body.c_str() << "', " << mn->item << ", "
					<< (uint32)mn->time << ", " << mn->money << ", " << 0 << ", "
					<< mn->checked << " )";
...
void
WorldSession::HandleAuctionSellItem( WorldPacket & recv_data )
{
...
void
WorldSession::HandleAuctionListOwnerItems( WorldPacket & recv_data )
{
	WorldPacket data;
	uint64 auctioneer;
	recv_data >> auctioneer;
...
void
WorldSession::HandleAuctionListItems( WorldPacket & recv_data )
{
