...
uint32 Quest::GenerateQuestXP( Player *pPlayer )
{
	...
	if( mmx <= (pPlayer->GetLevel()*5))
	{
		srand ( time(NULL) );
		return (int)rand()%(pPlayer->GetLevel()*5);
	}
...
bool Quest::QuestIsTakable( Player *pPlayer )
{
	...
	if(QuestIsCompatible( pPlayer ))
	{
		sLog.outDebug("QUEST:: Quest [%u] is compat.\n", this->m_questId );
		if(QuestPreReqSatisfied( pPlayer ))
		{
			sLog.outDebug("QUEST:: Quest [%u] is prereq is ok.\n", this->m_questId );
			return true;
		}
		else
		{
			//sLog.outDebug("QUEST:: Quest [%u] is prereq are NOT ok.\n", this->m_questId );
			return false;
		}
	}
	else
	{
		//sLog.outDebug("QUEST:: Quest [%u] is  NOT compat.\n", this->m_questId );
		return false;
	}

	//sLog.outDebug("QMCHECK: Quest '%u' can be taken.\n", this->m_questId );
...
bool Quest::QuestCanShowUnsatified( Player *pPlayer )
{
	...
	if ( iPLevel < m_requiredLevel ) return true;
