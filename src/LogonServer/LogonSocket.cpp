...
// Checking account:
	std::stringstream ss;
	ss << "SELECT acct, password, gm, banned FROM accounts WHERE login='" << _login << "'"; // password
	...
	uint32 acctId = (*result)[0].GetUInt32();
	_password = (*result)[1].GetString();
	uint16 security = (*result)[2].GetUInt16();
	bool banned = (*result)[3].GetBool();
...
// Checking players count:
	if (banned)
	{
		_pending_error = LOGIN_BANNED;
		return;
	}
	else if (sWorld.FindSession(acctId))
	{
		_pending_error = LOGIN_ALREADYONLINE;
		return;
	}
	else if (sWorld.GetPlayerLimit() > 0 &&
...
// Getting account info:
	...
	ss.rdbuf()->str("");
	ss << "SELECT COUNT(*) FROM characters WHERE acct=" << id;
...
// Building packet:
	...
	for( i = sRealmList.begin( ); i != sRealmList.end( ); ++i )
	{
		...
		pkt << (float) i->second->population; // Population value.
		pkt << (uint8) chars; 
		pkt << (uint8) i->second->timezone;
		pkt << (uint8) 0xC8; // can be 0x48 - End Of Realm Description
	}
...
// Building final packet:
	...
	sLog.outDebug ("[RealmList] Sent List");
...
// find total count of patches:
	...
	{
		sLog.outError("   |::Error: patches dir is empty");
		return;
	}
	else
	{
...
// loading info:
	...
	else
	{
		for (int i=0; findnext(hFind, &fd)==0;)
			if ( (fd.attrib & A_SUBDIR) == 0)
			{
				...
				if (processed != 5)
				{
					sLog.outError("   |::Error: \"%s\" has incorrect name format!", fd.name);
					--patches_count;
					continue;
				}
				sLog.outMenu("   |::Found %d [%d%c%c%c%c]\n",
					i+1,
					patches[i].build,
					patches[i].localization[3],
					patches[i].localization[2],
	...
	sLog.outString(" ::Found %d patches.\n", patches_count);
#else // ndef _WIN32
	patches_count = 0;
	patches		  = NULL;
#endif
...
{
	int accepted_versions[]=_VERTARGET;
    for(int i=0;accepted_versions[i]; ++i)
	{
        if(build==accepted_versions[i])
		{
			sLog.outDebug ("[Auth] Build accepted: %d", build);
            return LOGIN_OK;
		}
		else if (HavePatch(build, localization))
		{
			sLog.outDebug ("[Auth] Build has patch: %d", build);
			return LOGIN_DOWNLOADFILE;
		}
	}
	sLog.outDebug ("[Auth] Build unknown: %d", build);
