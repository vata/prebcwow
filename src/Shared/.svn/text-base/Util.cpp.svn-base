...
using namespace std;
//------------------------------------------------------------------------------
char *StrCompile(const char *str, ...)
{
...
{
	if((progressPos%10) == NULL)
	{
		printf ("\r%s [", label);
		uint32 barPos = (progressPos * 50 / progressTotal + 1);
		for (uint32 p = 0; p < barPos; ++p) putchar ('þ');
		for (uint32 p = barPos; p < 50; ++p) putchar (' ');
		int percentage = (barPos-1)*2;
		printf ("] %d%% ", percentage);
		switch(percentage % 3)
		{
		case NULL:	printf("Ûþþ"); break;
		case 2:		printf("þÛþ"); break;
		default:	printf("þþÛ"); break;
		}
		printf("   %d/%d\r", progressPos, progressTotal);
		fflush(stdout);
	}
}
//------------------------------------------------------------------------------
