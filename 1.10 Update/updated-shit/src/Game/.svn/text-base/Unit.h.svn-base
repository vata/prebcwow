...
//-----------------------------------------------------------------------------
typedef HM_NAMESPACE::hash_map<Unit *, float> TargetMap;	// Collection of creature targets
...
public:
	...
	inline void
		AddHate_NR (Unit *unit, float hate)
	{
		bool wasInCombat = isInCombat();
		ResetHateDegradeCounter();
		m_targets[unit] = GetHate (unit) + hate;
	...
	//
	inline void
		ShareHateWith (Unit *unit, float scale=1.0f)
	{
		for (TargetMap::iterator itr = m_targets.begin(); itr != m_targets.end(); itr++)
		{
	...
	//--------------------------------
	inline int32 GetAttackPower() { return (int32)GetUInt32Value (UNIT_FIELD_ATTACK_POWER); }
	inline void ModifyAttackPower (int32 d) { SetAttackPower (GetAttackPower() + d); }
	void SetAttackPower (int32 value);

	inline int32 GetRangedAttackPower() { return (int32)GetUInt32Value (UNIT_FIELD_RANGED_ATTACK_POWER); }
...
public:
	...
	void SetStealthLevel(int32 stealth) { m_stealthLevel = stealth; }
