...
Creature::Creature() : Unit()
{
	...
	// Temp fix ( maybe a try )
	/*if ( isSpiritHealer() )
		SetUInt32Value( UNIT_FIELD_FLAGS, UNIT_FLAG_SPIRITHEALER );
	}*/
}
...
void
Creature::Say(uint64 receiver, const char *text, Language language)
{
	CreatureTemplate * cr = this->GetCreatureTemplate();
...
void
Creature::Flee (Unit *from, int seconds, int helpRadius)
{
...
void
Creature::UpdateMobMovement( int32 p_time)
{
	int32 timediff = 0;
	...
	if (m_flee > FLEE_STATE_NONE)
	{
		...
		if (!m_moveTimer)
		{
			...
			if (m_timeMoved > m_timeToMove)
			{
				...
				sLog.outDebug ("AI: %X Fleeing, next random point (%.1f, %.1f)",
					GetGUIDLow(), dx, dy);
		...
		if (m_flee == FLEE_STATE_FLEE && m_attackTimer == 0)
		{
			...
			if ( isInCombat() )
			{
				sLog.outDebug ("AI: %X Flee timer finished, now attacking back", GetGUIDLow());
			} else {
				sLog.outDebug ("AI: %X Flee timer finished, nobody to attack back", GetGUIDLow());
				return;
			}
		}
	}
...
void
Creature::ResetRespawnTimer (uint32 time1, uint32 time2)
{
	if (time1)
		m_spawnTime[0] = time1;
...
Creature::Update( int32 p_time )
{
    ...
    if(m_deathTimer > 0)
    {
        ...
        {
            // time to respawn!
            sLog.outDetail("Removing corpse...");
...
void
Creature::Create (uint32 guidlow, const char* name, uint32 mapid, float x, float y, float z, float ang)
{
...
/// Quests
uint32
Creature::getDialogStatus(Player *pPlayer, uint32 defstatus)
{
	bool wasReward  = false;
	bool wasRewardRep  = false;
	bool wasAvail   = false;
	bool wasIncompl = false;
	bool wasUnavail = false;
   ...
   for( list<uint32>::iterator i = mInvolvedQuestIds.begin( ); i != mInvolvedQuestIds.end( ); ++ i )
    ...
    return DIALOG_STATUS_CHAT; // was NONE
}

Quest*
Creature::getNextAvailableQuest(Player *pPlayer, Quest *prevQuest)
{
    uint32 quest_id;
    uint32 status;
...
void
Creature::prepareQuestMenu( Player *pPlayer, NPCQuestMenu *qMenu )
{
    uint32 quest_id;
    uint32 status;
    Quest *pQuest;
	...
	for( list<uint32>::iterator i = mInvolvedQuestIds.begin( ); i != mInvolvedQuestIds.end( ); ++ i )
    {
		...
		// Ignore any that have been completed
		if ( status == QUEST_STATUS_COMPLETED )
		{
			sLog.outDebug( "Ignoring quest %s, because it has been flagged as completed.", pQuest->m_title.c_str());
		}
	}
}
...
bool
Creature::hasInvolvedQuest(uint32 quest_id)
{
    for( list<uint32>::iterator i = mInvolvedQuestIds.begin( ); i != mInvolvedQuestIds.end( ); ++ i )
    {
        if (*i == quest_id)
...
/// Looting
void
Creature::SetEmptyLoot (uint32 loot_sparkles)
{
	item_list.clear();
	m_lootMoney = 0;
...
Creature::GenerateLoot()
{
	...
	if (iter == objmgr.m_lootTemplates.end()) {
		sLog.outDebug ("LOOTGEN: No loot template for creature %d", GetEntry());
		return;
	}
	...
	uint64 KillerGUID = GetAttacker()->GetGUID();
	Quest *questtocheck;
	bool qItemEnabled;
	uint32 Quest_ItemId = 0;
	int icount;
	...
	for (uint32 i = 0; i < pvec->size(); ++i)
	{
		...
		if (itemProto->Class == 12) {
			...
			sLog.outDebug ("LOOTGEN: Item=%d Chance=%.6f Creature=%d - skipped quest item", itemId, chance, GetEntry());
						sLog.outString("Last Killer for creature: %u", KillerGUID);
			// todo: Make sure killer is a player
			
			stringstream ss;
			ss << "SELECT * FROM queststatus WHERE playerId='";
			ss << KillerGUID;
			ss << "' AND status='3'";
			sLog.outString("DB: %s", ss.str().c_str());
			...
			do
			{
				...
				questtocheck = objmgr.GetQuest(fields[1].GetUInt32());
				// sLog.outString("Quest %u - %s, checking for loot", questtocheck->m_questId, questtocheck->m_title.c_str());
				/// sLog.outString("ItemCount: %u, %u", questtocheck->m_questItemCount, questtocheck->m_questItemId);
				for (icount = 0; icount < 5; icount++)
				{
					if (questtocheck->m_questItemId[icount] == itemId)
					{
						qItemEnabled = true;
						addItem(itemId, 1);
						continue;
		...
		if (randomizer.randf() < chance / 100.0f) {
			sLog.outDebug ("LOOTGEN: Item=%d Chance=%.6f Creature=%d - ADDED To Loot", itemId, chance, GetEntry());
...
void
Creature::AI_AttackReaction (Unit *pAttacker, float damage_dealt)
{
    WPAssert(pAttacker);
...
Creature::AI_Update()
{
    ...
    for (TargetMap::iterator itr = m_targets.begin(); itr != m_targets.end(); )
    {
        ...
        if (! pVictim->isAlive())
		{
			sLog.outDebug ("AI-Hate: %X hated %X, but its dead, stopped hate",
        ...
        /*if (lengthSq > 30.0f * 30.0f)
		{
			...
			sLog.outDebug ("AI-Hate: %X hated %X, but its too far, stopped hate",
				this->GetGUIDLow(), pVictim->GetGUIDLow());
            RemoveHate (pVictim);
			pVictim->RemoveHate (this);
			goto continuehere;
...
continuehere:
	...
	{
		sLog.outDebug ("AI: Finished combat, restoring self");
		OnExitCombat();
	}
	...
	if (m_hateDegradeCounter > -1 && GetDistanceSq (mostHated) > 9.0f) {
		...
		if (wasInCombat && m_hateDegradeCounter < 0)
		{
			...
			//
			sLog.outDebug ("AI-Hate: %X most hated victim ran away, stopped hate",
				this->GetGUIDLow());
			//ClearHate();
	...
	if (mostHated != NULL && ! isFleeing())
    {
        ...
        //if (targetDistance > (reach + radius) /*&& m_movementState != AI_STATE_MOVING*/)
        {
			...
			//sLog.outDebug ("AI: %X hates %X but needs to run closer",
        ...
        else
        {
            if(m_creatureState != AI_STATE_ATTACKING)
            {
				...
				//sLog.outDebug ("AI: %X hates %X and starts attacking",
				//	this->GetGUIDLow(), mostHated->GetGUIDLow());
...
void
Creature::AI_SendMoveToPacket(float x, float y, float z, uint32 time, bool run)
{
	if (isStunned()) return;
...
void
Creature::AI_MoveTo(float x, float y, float z, bool run, float stopDistance)
{
	if (isStunned()) return;
	...
	*/
	...
	//sLog.outDebug ("AI_MoveTo: from (%.1f,%.1f) to (%.1f,%.1f), move time %d ticks, "
...
void
Creature::AI_MoveTo (Unit *npc, bool run, float stopDistance)
{
	AI_MoveTo (npc->GetPositionX(), npc->GetPositionY(), npc->GetPositionZ(), run, stopDistance);
}

void
Creature::AI_Follow (Unit *target)
{
	UnitToFollow = target;
}

void
Creature::AI_StopFollow ()
{
	UnitToFollow = NULL;
}

bool
Creature::addWaypoint(float x, float y, float z, uint8 wait1, uint8 wait2)
{
...
bool
Creature::insertWaypoint (float x, float y, float z, int pos, uint8 wait1, uint8 wait2)
{
	if(m_waypoints.size() + 1 >= MAX_CREATURE_WAYPOINTS) {
...
void
Creature::LoadFromDB (uint32 guid, WaypointVectorMap *precache)
{
	stringstream ss;
...
bool
Creature::SpawnCreatureFromDB(Field *fields, WaypointVectorMap *precache)
{
	// CreatureTemplate	*ct;
	...
	uint32	Level,
			levelDelta = ct->Level[1] - ct->Level[0];
	...
	SetZoneId (objmgr.LookupZoneId (mapId, x, y));
	SetUInt32Value (OBJECT_FIELD_ENTRY, ct->Entry);
	SetFloatValue (OBJECT_FIELD_SCALE_X, ct->Size);
	SetUInt32Value (UNIT_FIELD_DISPLAYID, ct->Model);
	SetUInt32Value (UNIT_FIELD_NATIVEDISPLAYID, ct->Model);
	SetUInt32Value (UNIT_NPC_FLAGS, ct->NPCFlags);
	SetUInt32Value (UNIT_FIELD_FACTIONTEMPLATE , ct->Faction);
	SetHealth (ct->MaxHealth);
	SetMaxHealth (ct->MaxHealth);
	SetLevel ((uint8)Level);
	SetFloatValue (UNIT_FIELD_COMBATREACH, ct->CombatReach);
	SetMinDamage ((float)ct->Damage[0]);
	SetMaxDamage ((float)ct->Damage[1]);
	SetUInt32Value (UNIT_FIELD_BASEATTACKTIME, ct->Attack[0]);
	SetUInt32Value (UNIT_FIELD_BASEATTACKTIME+1, ct->Attack[1]);
	SetFloatValue (UNIT_FIELD_BOUNDINGRADIUS, ct->BoundingRadius);
	m_creatureTemplate = objmgr.GetCreatureTemplate (GetEntry());
	SetUInt32Value(UNIT_FIELD_FLAGS, m_creatureTemplate->Flags1);
    m_zoneId = objmgr.LookupZoneId (mapId, x, y);
	LoadValues(fields[7].GetString());
...
void
Creature::SetUpCreature(uint8 MovementType, bool MoveRun, uint32 SpawnTime1, uint32 SpawnTime2, float SpawnDist, uint32 CurrentWaypoint, float DestinationX, float DestinationY, float DestinationZ, float Orientation, WaypointVectorMap *precache, bool LoadOtherShit)
{

	m_movementType = (Creature::MovementType)MovementType;
    m_moveRun = MoveRun;
	m_spawnTime[0] = SpawnTime1;
	m_spawnTime[1] = SpawnTime2;
	m_spawnDist = SpawnDist;
	m_currentWaypoint = CurrentWaypoint;

	m_destinationX = respawn_cord[0] = DestinationX;
	m_destinationY = respawn_cord[1] = DestinationY;
	m_destinationZ = respawn_cord[2] = DestinationZ;
	m_orientation = respawn_angle = Orientation;
	SetPosition (m_destinationX, m_destinationY, m_destinationZ, m_orientation);

	if (!LoadOtherShit)
		return;
	
	// sLog.outString("Doing other shit.");
    if ( HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_VENDOR ) )
        _LoadGoods();
...
Creature::SetUpCreatureFull(uint32 ID, float XPos, float YPos, float ZPos, float Orientation, uint32 MapId, uint32 Entry, uint8 MovementType, bool MoveRun, uint32 SpawnTime1, uint32 SpawnTime2, float SpawnDist, uint32 CurrentWaypoint, float DestinationX, float DestinationY, float DestinationZ, float DOrientation, WaypointVectorMap *precache, bool LoadOtherShit)
{
	...
	Create (objmgr.GenerateLowGuid (HIGHGUID_UNIT), ct->Name.c_str(), MapId, XPos, YPos, ZPos, Orientation);
	float x = XPos;
	float y = YPos;
	float z = ZPos;
	...
	SetUInt32Value (UNIT_FIELD_BASEATTACKTIME+1, ct->Attack[1]);
	SetFloatValue (UNIT_FIELD_BOUNDINGRADIUS, ct->BoundingRadius);
	m_creatureTemplate = objmgr.GetCreatureTemplate (Entry);
	// SetUInt32Value(UNIT_FIELD_FLAGS, m_creatureTemplate->Flags1);
	...
	m_destinationY = respawn_cord[1] = DestinationY;
	m_destinationZ = respawn_cord[2] = DestinationZ;
	m_orientation = respawn_angle = DOrientation;
	SetPosition (m_destinationX, m_destinationY, m_destinationZ, m_orientation);
    ...
    if(result)
    {
        do
        {
            ...
            if (getItemCount() >= MAX_CREATURE_ITEMS)
            {
                ...
                sLog.outError( "Vendor %u has too many items. Check the DB!", GetGUIDLow() );
                break;
            }
...
void
Creature::_LoadMovement (WaypointVectorMap *precache)
{
	// clean waypoint list
	//m_nWaypoints = 0;
	m_waypoints.clear();
...
bool
Creature::RelatedByFirstName (Creature *cr)
{
...
bool
Creature::SearchForHelpfulFamily (float senseRadius)
{
	...
	for (itr = m_mapCell->Begin(); itr != m_mapCell->End(); ++itr)
	{
		...
		if ((*itr)->GetTypeId() == TYPEID_UNIT)
		{
			...
			//sLog.outDebug ("AI: Sharing hate with helpful family member %X",
			//	cr->GetGUIDLow());
...
void
Creature::GuardCheckForViolators()
{
	...
	for (itr = m_mapCell->Begin(); itr != m_mapCell->End(); itr++)
	{
			...
			//if (cr != NULL)
		{
			...
			/*sLog.outDebug ("AI: Thing %X touched aggro radius of guard %X",
...
Creature::GenerateWorldLoot()
{
    ...
    for (uint32 i = 0; i < pvec->size(); ++i)
	{
		...
		if (itemProto->Class == 12) {
			sLog.outDebug ("WORLDLOOT: SKIP QUEST ITEM Item=%d Chance=%.6f Creature=%d", item_id, chance, GetEntry());
			continue;
		}
		///////////////////
		...
		if (monster_level <= GetLevel()) {
			...
			if ((GetLevel() - monster_level) < 5 && GetElite() == elite) {
				...
				if (randomizer.randf() < chance / 100.0f) {
					sLog.outDebug ("WORLDLOOT: ADD Item=%d ReqLevel=%d Chance=%.6f CrID=%d Elite=%d", item_id, monster_level, chance, GetEntry(), elite);
