...
bool
Unit::CanReachWithAttack (Unit *pVictim)
{
...
void
Unit::UnitDeath (Unit *pKiller, bool isCritterVictim)
{
	...
	// Last killer - for quest item loot generation.
	((Creature*)this)->SetLastKillerGUID(pKiller->GetGUID());
	((Creature*)this)->SetLastKiller(pKiller);
	...
	if (pKiller->isPlayer() && pKiller != this)
	{
		...
		{
			sLog.outDebug ("Killer GUID=%X is not an initial tagger GUID=%X "\
...
//-----------------------------------------------------------------------------
void
Unit::DealDamage(Unit *pVictim, uint32 damage)
{
	((Creature*)pVictim)->SetAttackerGUID(this->GetGUID());
	((Creature*)pVictim)->SetAttacker(this);

	//----- Damage Reduction ---------------
	if (isUnit() && pVictim->isPlayer())
		damage = damage - CalcDamageReduction(pVictim, this, damage);
...
//================================================================================================
void
Unit::SpellNonMeleeDamageLog(Unit *pVictim, uint32 spellID, uint32 damage)
{
    if(!this || !pVictim)
        return;
    if(!this->isAlive() || !pVictim->isAlive())
        return;
    sLog.outDetail("AttackerStateUpdate: %u %X attacked %u %X for %u dmg infliced by %u",
...
void
Unit::PeriodicAuraLog(Unit *pVictim, uint32 spellID, uint32 damage, uint32 damageType)
{
    if(!this || !pVictim)
        return;
    if(!this->isAlive() || !pVictim->isAlive())
        return;
    sLog.outDetail("PeriodicAuraLog: %u %X attacked %u %X for %u dmg infliced by %u",
        GetGUIDLow(), GetGUIDHigh(), pVictim->GetGUIDLow(), pVictim->GetGUIDHigh(), damage, spellID);
...
void
Unit::AttackerStateUpdate (Unit *pVictim, uint32 damage, bool DoT)
{
    uint32 spell = 0;
	...
	float	chanceToHit = 100.0f;
		...
		//-----------------------------------------------------------
	// Calculates when a players weaponskill needs to level up
	//-----------------------------------------------------------
	if (isPlayer()) {
		...
		double multiplyer = 0.6;

		//pre selecting skill that needs an update
		if (player->GetItemBySlot(EQUIPMENT_SLOT_MAINHAND)) 
		{
			item = player->GetItemBySlot(EQUIPMENT_SLOT_MAINHAND);
			skill_id = item->GetSkillNeeded(item->GetProto());
			item_id  = item->GetProto()->ItemId;
			item_name  = item->GetProto()->Name1;
			sLog.outDebug("SKILL:: Player is using item [%s] with id [%i] that uses skill [%i]",item_name.c_str(),item_id,skill_id);
		...
		else
		{
			...
			{
				player->AddSkill(162, 1, (player->GetLevel()*5)+5);
			}
			skill_id = 162;
		}

		mySkill  = player->GetSkillLevel(skill_id);

		//----------------------------------------------------------------
		// Begin skill calculation
		//----------------------------------------------------------------
		if (player->HasSkill(skill_id))
		{
				
			uint32 skill_level = player->GetSkillLevel(skill_id);
				
			sLog.outDebug("SKILL:: Player current skill level in skill [%i] is [%i/%i]",skill_id,skill_level,player->GetSkillMax(skill_id));
							
			// Gives skill when players needs it!
			if(skill_level >= player->GetSkillMax(skill_id))
			{
				//if for some reason player has to high skill
				if(skill_level >= player->GetSkillMax(skill_id))
				{
						player->UpdateSkill(skill_id,
											player->GetSkillMax(skill_id),
											player->GetSkillMax(skill_id));
				}

				sLog.outDebug("SKILL:: Has max skill level for current weapon type");
			}
			else if(skill_level < (player->GetSkillMax(skill_id)*0.5))
			{
				sLog.outDebug("SKILL:: Player has skill [%i] and should get new skill level [%d]",skill_id,(skill_level+1));
				player->UpdateSkill(skill_id,
								   (skill_level+1),
									player->GetSkillMax(skill_id));
			}
			else
			{
				...
				sLog.outDebug("SKILL:: First skill formule did not apply");
				// check if next skill is set, and if it is bigger then 0
				if(player->HasNextSkill(skill_id) && player->GetNextSkill(skill_id) > 0)
				{
					sLog.outDebug("SKILL:: HasNextSkill() is set");
					// levels up a next slash
					if(player->GetNextSkill(skill_id) == 1)
					{
						player->UpdateSkill(skill_id,
										   (skill_level+1),
										   (player->GetSkillMax(skill_id)));

						sLog.outDebug("SKILL:: Player got new skill level");
							
						//set next skill to zero so next slash wel get cought by the else satement
						player->DelNextSkill(skill_id);

					}
					else
					{
						player->SetNextSkill(skill_id,player->GetNextSkill(skill_id)-1);
						sLog.outDebug("SKILL:: Player needs [%d] slashes until he levels!",player->GetNextSkill(skill_id));
					}
				}
				//if has not  yet been set
				else
				{
					//save next skill info in array
					// new formule 1000/(1+55*0.975^x)
                    //player->SetNextSkill(skill_id,skill_level*(player->GetLevel()/**multiplyer*/));
					//player->SetNextSkill(skill_id,ftoi(1000/(1 + (55*topower((float)0.975,skill_level)) ) ) );
					player->SetNextSkill(skill_id, (skill_level*player->GetLevel()) + (player->GetSkillMax(skill_id)/3)  );
					if(player->GetNextSkill(skill_id) > 750)
					{
						player->SetNextSkill(skill_id,750);
					}
					sLog.outDebug("SKILL:: No next_skill[] was set, player will level in [%d] slashes",player->GetNextSkill(skill_id));
				}
			}
		}
	}
    ...
    //sLog.outDetail("AttackerStateUpdate: %u %X attacked %u %X for %u dmg.",
...
void
Unit::smsg_AttackStop(uint64 victimGuid)
{
    WorldPacket data;
    ...
    data << uint32( 0 );
    SendMessageToSet(&data, true);
	sLog.outDebug ("WORLD: SMSG_ATTACKSTOP");
    sLog.outDetail("LG[%u] HG[%X] stopped attacking "I64FMT, GetGUIDLow(), GetGUIDHigh(), victimGuid);
}

void
Unit::smsg_AttackStart(Unit* pVictim)
{
    ...
    data << pVictim->GetGUID();
    SendMessageToSet(&data, true);
    sLog.outDebug( "WORLD: SMSG_ATTACKSTART" );
}

bool
Unit::AddAffect(Affect *aff, bool uniq)
{
    AffectList::const_iterator i;

    sLog.outDetail("Unit::AddAffect() - adding affect");
    ...
    if (i != m_affects.end())
    {
        if(i != 0)
            (*i)->SetDuration(0);
        sLog.outDetail("Unit::AddAffect() - overwriting Affect");
    }
    ...
    sLog.outDetail("Unit::AddAffect() - affect added");

    return true;
}

void
Unit::RemoveAffect(Affect *aff)
{
...
void
Unit::RemoveAffectById(uint32 spellId)
{
    if(m_aura != NULL)
...
bool
Unit::SetAffDuration(uint32 spellId,Unit* caster,uint32 duration)
{
    AffectList::iterator i;
...
bool
Unit::RemoveAffect(uint32 spellId)
{
...
void
Unit::_AddAura(Affect *aff)
{
    ASSERT(aff);

    if (!aff->GetId())
    {
        sLog.outDetail("Unit::_AddAura() - affect doesn't have id");
        return;
    }
    ...
    if (slot == 0xFF)
    {
        sLog.outDetail("Unit::_AddAura() - no free aura slots found");
        return;
    }

    sLog.outDetail("Unit::_AddAura() - found slot %u", (uint32)slot);
    ...
    sLog.outDetail("Unit::_AddAura() - old value: 0x%08X", value);
    ...
    sLog.outDetail("Unit::_AddAura() - new value: 0x%08X", value);
...
float
GetDistance (float xe, float ye, float xz, float yz)
{
	return sqrt( ( xe - xz ) * ( xe - xz ) + ( ye - yz ) * ( ye - yz ) );
}

float
GetDistanceSq (float xe, float ye, float xz, float yz)
{
	return  ( xe - xz ) * ( xe - xz ) + ( ye - yz ) * ( ye - yz );
}

float
GetAngle (float xe, float ye, float xz, float yz)
{
        float w;
        w = atan( ( yz - ye ) / ( xz - xe ) );
        w = ( w / (float)DEG2RAD );
...
bool
InArc( float radius,  float xM, float yM, float offnung, float orientation, float xP, float yP )
{
...
bool
Unit::isInFront(Unit* target, float distance)
{
...
void
Unit::OnTakeDamage (Unit *attacker) 
{
			...
			j != aff->GetModList().end(); ++j)
		{
			...
			switch (j->GetType()) {
				case SPELL_AURA_TRANSFORM:
					RemoveAffect (aff);
					sLog.outDetail("UNIT: Taken damage, transform removed");
					return;
				case SPELL_AURA_MOD_STUN:
					...
					{
						RemoveAffect (aff);
						sLog.outDetail("UNIT: Taken damage, stun removed");
						return;
					} 
					break;
			}
...
inline float
cubicPolynome (int level, float a3, float a2, float a1, float a0) 
{
...
//
void
Unit::_ApplyLevelUpAttributes (int32 level, bool apply)
{
	uint8	cls = GetClass();
...
void
Unit::SetStrength (int32 value)
{
	//sLog.outDetail ("Setting Strength to %d", value);
...
void
Unit::SetAgility (int32 value) 
{
	//sLog.outDetail ("Setting Agility to %d", value);
...
void
Unit::SetStamina (int32 value)
{
	//sLog.outDetail ("Setting Stamina to %d", value);
...
void
Unit::SetIntellect (int32 value)
{
	//sLog.outDetail ("Setting Intellect to %d", value);
...
void
Unit::OnModifyIntellect (int32 d)
{
...
void
Unit::SetAttackPower (int32 value)
{
	int32	d = value - GetAttackPower();
	SetUInt32Value (UNIT_FIELD_ATTACK_POWER, value);
	...
	//sLog.outDebug ("SetAttackPower (value=%d): Boosted damages by %.1f (attack speed %d) - result %.1f..%.1f",
...
void
Unit::SetRangedAttackPower (int32 value)
{
	int32	d = value - GetRangedAttackPower();
	SetUInt32Value (UNIT_FIELD_RANGED_ATTACK_POWER, value);
	...
	sLog.outDebug ("SetRangedAttackPower (value=%d): Boosted damages by %.1f (attack speed %d) - result %.1f..%.1f",
...
Unit::CanSee (Object *o)
{
	...
	// Dead units can see spirit healers. Live units never see them
...
void
Unit::VirtualEquip (uint8 count, uint32 slot, uint32 model, uint32 data )
{
...
uint32
Unit::CalculateDamage (uint32 &additionalSpellId, uint32 &additionalSpellDmg)
{
	...
	//sLog.outDebug ("CalculateDamage: min = %.1f, max = %.1f, result = %d",
...
void
Unit::Say (Unit *receiver, const char *text, Language language)
{
	WorldPacket		data;
...
void
Unit::MinimapTrackingStatus( bool bEnabled )
{
